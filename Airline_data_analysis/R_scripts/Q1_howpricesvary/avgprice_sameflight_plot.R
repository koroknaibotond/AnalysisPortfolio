ggplot(data = average_prices_samecat) +
  geom_col(mapping = aes(x = airline, y = Average_Price, fill = airline)) +
  facet_wrap(~departure_time, scales = "free_x") +
  labs(
    x = "Airline Name",
    y = "Average Price (Indian Rupees)",
    title = "Average Flight Price by Airline and Departure Time",
    subtitle = "Analysis of flights between Delhi and Mumbai"
  ) +
  theme(
    plot.title = element_text(hjust = 0.5, size = 16, face = "bold", family = "serif"),
    plot.subtitle = element_text(hjust = 0.5, size = 12, face = "bold", family = "serif"),
